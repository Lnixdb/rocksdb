...
IOStatus ZondaFileSystem::NewWritableFile(const std::string& fname, const FileOptions& options,
...
IOStatus ZondaFileSystem::NewRandomRWFile(const std::string& fname, const FileOptions& options,
...
IOStatus ZondaFileSystem::NewDirectory(const std::string& name, const IOOptions& opts,
                      std::unique_ptr<FSDirectory>* result,
                      IODebugContext* dbg)  {
  return base_fs_->NewDirectory(name, opts, result, dbg);
}

IOStatus ZondaFileSystem::FileExists(const std::string& fname, const IOOptions& opts,
                    IODebugContext* dbg)  {
  return base_fs_->FileExists(fname,  opts, dbg);
}

IOStatus ZondaFileSystem::GetChildren(const std::string& dir, const IOOptions& opts,
                     std::vector<std::string>* result,
                     IODebugContext* dbg)  {
  return  base_fs_->GetChildren(dir, opts, result, dbg);
}

IOStatus ZondaFileSystem::DeleteFile(const std::string& fname, const IOOptions& opts,
                    IODebugContext* dbg)  {
 return base_fs_->DeleteFile(fname, opts, dbg);
}

IOStatus ZondaFileSystem::CreateDir(const std::string& name, const IOOptions& opts,
                   IODebugContext* dbg)  {
...
IOStatus ZondaFileSystem::DeleteDir(const std::string& name, const IOOptions& opts,
                   IODebugContext* dbg)  {
  return base_fs_->DeleteDir(name, opts, dbg);
}

IOStatus ZondaFileSystem::GetFileSize(const std::string& fname, const IOOptions& opts,
...
IOStatus ZondaFileSystem::RenameFile(const std::string& src, const std::string& target,
                    const IOOptions& opts,
                    IODebugContext* dbg)  {
  return base_fs_->RenameFile(src, target, opts, dbg);
}

IOStatus ZondaFileSystem::LinkFile(const std::string& src, const std::string& target,
                  const IOOptions& opts,
                  IODebugContext* dbg)  {
  return base_fs_->LinkFile(src, target, opts, dbg);
}

IOStatus ZondaFileSystem::NumFileLinks(const std::string& fname, const IOOptions& opts,
                      uint64_t* count, IODebugContext* dbg)  {
  return base_fs_->NumFileLinks(fname, opts, count, dbg);
}

IOStatus ZondaFileSystem::AreFilesSame(const std::string& first, const std::string& second,
                      const IOOptions& opts, bool* res,
                      IODebugContext* dbg)  {
  return base_fs_->AreFilesSame(first, second, opts, res, dbg);
}

IOStatus ZondaFileSystem::LockFile(const std::string& fname, const IOOptions& opts,
...
IOStatus ZondaFileSystem::GetTestDirectory(const IOOptions& opts, std::string* result,
                          IODebugContext* dbg)  {
  return base_fs_->GetTestDirectory(opts, result, dbg);
}

IOStatus ZondaFileSystem::GetFreeSpace(const std::string& fname, const IOOptions& opts,
                      uint64_t* free_space,
                      IODebugContext* dbg)  {
  return base_fs_->GetFreeSpace(fname, opts, free_space, dbg);
}

IOStatus ZondaFileSystem::IsDirectory(const std::string& path, const IOOptions& opts,
